...
fn test_searing_onslaught_increases_dmg_every_3rd_use_per_round() {
    let mut gs = GameState::new(&vector![CharId::Diluc], &vector![CharId::Kaeya], true);
...
fn test_dawn_grants_pyro_infusion() {
    let mut gs = GameState::new(&vector![CharId::Diluc], &vector![CharId::Kaeya, CharId::Fischl], true);
    gs.ignore_costs = true;
    gs.advance_roll_phase_no_dice();
    gs.advance_multiple(&vec![
